...
Now that we have our data set-up properly we can begin with the pathway enrichment analysis. `netgsa` has two main functions. The first is `prepareAdjMat` which searches for edges in public databases and uses this information to estimate the adjacency matrices needed for `NetGSA`. The second is `NetGSA` which performs network-based gene set analysis. We will go into further details about the usage of these functions below.
...
The returned value from `prepareAdjMat` will always be in the correct format regardless of whether or not clustering is used, so one should always be able to pass `network_info[["Adj"]]` directly to `NetGSA`. The default is to use restriced maximum likelihood (REML) to calculate the variance component. However, this can be quite slow, so another option is to use the restricted Haseman-Elston regression (REHE) instead. This is beneficial since it allows for sampling so you can significantly increase the speed. For example:
